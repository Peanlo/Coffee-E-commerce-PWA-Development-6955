...
import { recoverAbandonedCart } from '../../services/abandonedCartService';
...
const CartPage = () => {
  ...
  useEffect(() => {
    const recoverCart = async () => {
      if (recoveryToken) {
        try {
          const { success, cartItems: recoveredItems, error } = await recoverAbandonedCart(recoveryToken);
          
          if (success && recoveredItems && recoveredItems.length > 0) {
            // Add recovered items to cart
            ...
            // Remove token from URL to prevent multiple recoveries
          ...
          toast.error('Failed to recover your cart');
        }
      }
    };
    
    recoverCart();
  }, [recoveryToken, addToCart]);
